...
//@WebServlet을 지정하면 따로 서블릿을 등록하고 url mapping하는 과정을 대신할 수 있다.
//1. 디렉토리 패턴: 디렉토리 형태로 서버의 해당 컴포넌트를 찾아서 실행하는 구조
// -> 정해진 url로만 서블릿에 요청을 보낼 수가 있다.
//2. 확장자 패턴: 확장자 형태로 서버의 해당 컴포넌트를 찾아서 실행하는 구조
// *.board -> 앞에 어떠한 경로가 존재하던지 .board 요청으로만 끝나면 해당 서블릿이 반응하겠다.
//@WebServlet("/board")
@WebServlet("*.board") //앞과 상관없이 .board면 모두 반응
public class FrontController extends HttpServlet {
	private static final long serialVersionUID = 1L; //신경쓸필요X
