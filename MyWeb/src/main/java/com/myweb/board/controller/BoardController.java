...
public class BoardController extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		case "regist" :
			...
			/*
			 Q. 왜 board_list.jsp로 바로 리다이렉트를 하면 안될까?
			 A. 데이터 베이스로부터 전체 글 목록을 가져오는 로직을 board_list.jsp에서는
			 작성하지 않을 것이기 때문. (jsp는 단순히 보여지는 역할만 수행)
...
//			response.sendRedirect("board/board_list.jsp");
		...
		case "list" :
			...
			/*
			 Q. 여기서 sendRedirect를 하면 안되는 이유
			 A. request객체에 list를 담아서 전달하려 하는데, sendRedirect를 사용하면
			 응답이 바로 나가면서 request 객체가 소멸한다.
