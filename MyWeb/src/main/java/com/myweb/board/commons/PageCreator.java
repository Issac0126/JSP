...
public class PageCreator {
	...
	//한 화면에 보여질 페이지 버튼 수
	private final int displatBtn = 10;

	//페이징 알고리즘을 수행할 메서드 선언.
	private void calcDataOfPage() {
		
		//보정 전 끝 페이지 구하기
		this.endPage = (int) Math.ceil(paging.getPage() / (double) displatBtn)
					*displatBtn;
		this.beginPage = this.endPage - displatBtn + 1;
